"""Module to simulate user interaction with Prim algorithm"""

from interface.core.base_program import BaseProgram
from interface.contents import content_dictionary
from tools.algorithms.prim import PrimSearch
from tools.api.object import Vertex

class PrimProgram(BaseProgram):
    """A class to provide a Prim-related operations to user."""

    def __init__(self) -> None:
        super().__init__()
        self.__prim = PrimSearch()
    ...
    def __execute_process(self):
        ...
        match option_input:
            ...
            case 2:
                ...
                return
            case 3:
                self.__prim.create_graph_from_problem_statement(2)
    ...
    def __run_graph(self, run: bool) -> str:
        ...
        run_graph = self.__prim.definition("prim")

        if run:
            tree = self.__prim.get_trees()[0]
            ...
            for edge in self.__prim.get_trees()[0]:
    ...
    def start(self):
        """A section to start the Prim program."""
        content = content_dictionary['prim']
    ...
    def __vertex_creation_session(self, label: str):
        ...
        vertex = self.__prim.add_vertex(label)
    ...
    def __create_vertex(self):
        ...
        while self._extrn.resume:
            self._refresh_display(title, self.__prim.definition('prim'))
    ...
    def __edge_content(self, src: Vertex | None, dest: Vertex | None) -> str:
        ...
        body: str = self.__prim.definition('prim')
        source_definition = None
        destination_definition = None

        if isinstance(src, Vertex):
            source_definition = src.definition('prim')

        source_prompt: str = '\n' + f"\tSource:\n\t  - {source_definition}"
        body += source_prompt

        if isinstance(dest, Vertex):
            destination_definition = dest.definition('prim')
    ...
    def __vertex_retrieval_session(self, search_for: str | None = None) -> Vertex | None:
        ...
        vertex = self.__prim.get_vertex(label_input)
    ...
    def __edge_creation_session(
        ...
        if two_direction_ctx:
            self.__prim.add_edge(src, dest, (weight_list[0], weight_list[-1]))
        ...
        self.__prim.add_edge(src, dest, weight_list[0])
    ...
    def __create_edge(self):
        ...
        if len(self.__prim.get_vertices()) < 2:
    ...
    def __check_edges(self) -> bool:
        ...
        for vertex in self.__prim.get_vertices():
            if vertex.get_edges():
                return True
